...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                onReceivedMessage(clientSocket, inputLine);
                if (inputLine.equalsIgnoreCase(getSessionEndWord())) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
            onClosedConnectionMessage(clientSocket);
        } catch (IOException e) {
    ...
    private static void handleErrorMessage(IOException e) {
        System.err.println("Error closing socket: " + e.getMessage());
    }

    private static void onClosedConnectionMessage(Socket clientSocket) {
        System.out.println("Connection closed with " + clientSocket.getInetAddress().getHostAddress() + ":"
                + clientSocket.getPort());
    }

    private static void onReceivedMessage(Socket clientSocket, String inputLine) {
        System.out.println("Received message: " + inputLine + " from "
                + clientSocket.getInetAddress().getHostAddress() + ":" + clientSocket.getPort());
    }

    protected String getSessionEndWord() {
        return "";
    }
}
